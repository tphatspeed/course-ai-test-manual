...
5. **Chờ page load xong:**
   ```
   browser_wait_for(<indicator page đã load>)
   ```

   > ⚠️ **KHÔNG gọi `browser_resize`** — viewport đã đúng từ `--viewport-size` lúc launch MCP server. Phóng viewport vượt cửa sổ OS làm trang bị cắt bên phải.
